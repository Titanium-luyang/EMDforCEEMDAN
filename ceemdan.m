...
for i=1:NR %calculates the first mode
    ...
    [temp, o, it]=EmdforCeemdan(temp,'MAXMODES',1,'MAXITERATIONS',MaxIter);
    temp=temp(1,:);
    aux=aux+temp/NR;
    iter(i,1)=it;
end;
...
while  nnz(diff(sign(diff(x-acum))))>2 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
                [temp, o, it]=EmdforCeemdan(x-acum+std(x-acum)*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
                temp=temp(1,:);
            catch
                it=0;
                temp=x-acum;
            ...
            [temp, o, it]=EmdforCeemdan(x-acum,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            temp=temp(1,:);
        end;
        aux=aux+temp/NR;
    iter(i,k+1)=it;    
